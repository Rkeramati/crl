...
class GridWorld:
    ...
    def reset(self):
        ...
        return self.to_idx(self.current_state)
    def to_idx(self, s):
        idx = s.y * self.height + s.x
        return idx
    ...
    def step(self, a):
        possible_moves = self.transitions(self.current_state)[a]
        #move = np.random.choice(np.arange(len(possible_moves)),\
        #        p=[x.prob for x in possible_moves])
        move = max(possible_moves, key=lambda t: t.prob)
        next_state = move #possible_moves[move]

        self.current_state = next_state.state
        if next_state.state in self.cliff_states \
                or next_state.state in self.goal_states:
            terminal = True
        else:
            terminal = False

        return self.to_idx(self.current_state), next_state.reward, terminal
