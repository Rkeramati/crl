...
class GridWorld:
    ...
    def observe(self, s, a, ns, r):
        # observe and update the count
        if s in self.counts.keys():
            if a in self.counts[s].keys():
                if ns in self.counts[s][a].keys():
                    self.counts[s][a][ns] += 1
                    self.reward[s][a] += r
                else:
                    self.counts[s][a].update({ns: 1})
                    self.reward[s][a] += r
            else:
                self.counts[s].update({a: {ns: 1}})
                self.reward[s].update({a: r})
        else:
            self.counts.update({s: {a: {ns: 1}}})
            self.reward.update({s: {a: r}})
    ...
    def transition_rmax(self, s, a):
        ...
        if s in self.counts.keys():
            if a in self.counts[s].keys():
                for key in self.counts[s][a].keys():
                    total += self.counts[s][a][key]
        if total < self.Rmax_m:
            return [Transition(state=self.magic_state, prob=1.0, reward=self.Rmax)]
        else:
            transitions = []
            for ns in self.counts[s][a].keys():
